...
export default function AuthButton({ locale: localeProp }: AuthButtonProps = {}) {
  ...
  return (
    <>
      ...
      {/* 모달: fixed inset-0로 전체 화면 고정, 내부 스크롤 */}
      {showModal && (
        <div
            ...
            <div className="min-h-full flex items-start sm:items-center justify-center p-4 sm:py-8">
              ...
              >
                ...
                <button
                  onClick={closeModal}
                  aria-label="닫기"
                  className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center rounded-full hover:bg-charcoal/5 text-charcoal/60 hover:text-charcoal"
                >
                  ✕
                </button>
